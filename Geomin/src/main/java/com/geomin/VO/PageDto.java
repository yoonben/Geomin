...
public class PageDto {
	...
	private RegionCri regioncri;		//페이지번호,페이지당 게시물수
	...
	public PageDto(RegionCri regioncri, int total){
		this.regioncri = regioncri;
		this.total = total;
		
		//페이지 블럭의 끝번호
		this.endNo = (int)(Math.ceil(regioncri.getPageNo()/5.0)* 5);
		
		//페이지 블록의 시작번호
		this.startNo = this.endNo - 4;
		
		//총 게시물의 수를 페이지당 보여지는 게시물의 수로 나눠서 실제 끝페이지 번호를 구함
		realEnd = (int)Math.ceil((total*1.0)/regioncri.getAmount());
